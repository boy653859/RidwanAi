...
export async function POST(req: Request) {
    try {
        ...
        if (!transcript || transcript.length > 20000) return NextResponse.json({ error: "Transcript must contain 1–20,000 characters." }, { status: 400 });
        if (!VOICES[name]) return NextResponse.json({ error: "Invalid voice." }, { status: 400 });
        if (!id || id.startsWith("YOUR-")) return NextResponse.json({ error: `Configure the Voice ID for "${name}" in route.ts.` }, { status: 400 });
        if (!RATES.has(sr) || speed < .1 || speed > 2 || volume < .5 || volume > 2) return NextResponse.json({ error: "Invalid output settings." }, { status: 400 });
    ...
    } catch (e) { console.error(e); return NextResponse.json({ error: e instanceof Error ? e.message : "Voice generation failed." }, { status: 500 }) }
}